...
export class DocumentationScraperService {
   ...
   */
  async scrapeAllDocumentation(): Promise<{ success: boolean; message: string; details: any }> {
    ...
    try {
      ...
      for (const source of this.documentationSources) {
        try {
          ...
          this.logger.error(`Error scraping ${source.name}: ${error.message}`, error.stack);
        }
      }
      ...
      this.logger.error(`Failed to complete documentation scraping: ${error.message}`, error.stack);
      return {
        success: false,
   ...
   */
  private async scrapeDocumentation(source: DocumentationSource): Promise<void> {
    this.logger.log(`Scraping documentation from ${source.name}: ${source.url}`);
  ...
  private async uploadDocumentationToSupabase(documentation: DocumentationData): Promise<void> {
    try {
      this.logger.log(`Uploading documentation for ${documentation.source_name} to Supabase`);
  ...
  async scheduleDocumentationScraping(intervalHours = 24): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to schedule documentation scraping: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async addDocumentationSource(source: DocumentationSource): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to add documentation source: ${error.message}`, error.stack);
      throw error;
    }
  }
}
